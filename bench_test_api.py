...
@app.post("/refresh")
def refresh_server_memory():
    print("\n[SERVER] Received refresh command. Purging persistent cache...")
    try:
        gc.collect()
        print("[SERVER] Core memory flush complete. Ready for next batch.")
        return {"status": "refreshed", "message": "Cache successfully cleared"}
    except Exception as e:
        print(f"[SERVER ERROR] Failed to clear memory cache: {e}")
...
async def predict(image:  UploadFile = File(...), prompt: str = Form(default="Describe this image in detail.")):
    ...
    try:
        ...
        generated_tokens = []
        async for chunk in run_llm_stream(embd_path, prompt, t0):
            if isinstance(chunk, bytes):
                generated_tokens.append(chunk.decode("utf-8", errors="ignore"))
            elif isinstance(chunk, str):
                generated_tokens.append(chunk)

        full_text_response = "".join(generated_tokens)

        return JSONResponse(
            status_code=200,
            content={
                "status": "success",
                "model": "custom-onnx-fastvlm-0.5b",
                "prompt": prompt,
                "response": full_text_response.strip()
            }
        )
        ...
        if embd_path and os.path.exists(embd_path):
            try:
                os.unlink(embd_path)
                print(f"[CLEANUP] Deleted temporary embeddings: {embd_path}")
            except Exception as ce:
                print(f"[WARNING] Could not delete temp file {embd_path}: {ce}")
