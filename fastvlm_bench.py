...
async def predict(
    ...
    try:
        ...
        # 3. Save embeddings to temp file
        embd_path = save_embeddings(embeddings)

        t3 = time.perf_counter()
        print(f"[TIMING] save embeddings: {(t3-t2)*1000:.1f} ms")

        # 4. Consume the generator synchronously to accumulate full text
        # Assumes run_llm_stream yields text chunks (strings)
        generated_chunks = []
        async for chunk in run_llm_stream(embd_path, prompt, t0):
            generated_chunks.append(chunk)
            
        full_response_text = "".join(generated_chunks).strip()

        t4 = time.perf_counter()
        print(f"[TIMING] text generation complete: {(t4-t3)*1000:.1f} ms | Total: {(t4-t0)*1000:.1f} ms")
        ...
        # Secure cleanup: Ensure the temporary embeddings file is unlinked 
        # even if the generation process throws an exception midway.
        if embd_path and os.path.exists(embd_path):
            try:
                os.unlink(embd_path)
            except Exception as clean_err:
                print(f"[WARNING] Failed deleting temp file {embd_path}: {clean_err}")
